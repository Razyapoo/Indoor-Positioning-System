...
def createDirectories(baseFolder, comparisonFolder, experimentName):
    """Create directories for saving plots."""
    folderToSave = os.path.join(baseFolder, comparisonFolder, experimentName)
...
for dataset in datasets:
    ...
    for compareWith in ['ref', 'uwb']:
        ...
        plotErrors(errorsDf, titleSuffix, fileName, folderToSave)
    
        frames = pd.read_csv(dataset["uwbPath"], sep=' ', header=None, usecols=[0])
        frames.columns = ['Frames']
    
        plotErrorTrend(errorsDf, frames, windowSize=50, titleSuffix=titleSuffix, fileName=fileName, folderToSave=folderToSave)
        ...
        plotDistanceErrors(distanceErrors, frames, titleSuffix, fileName, folderToSave)

                # Plot splits for scatter plots
        plotSplits(refSplits, uwbSplits, 'Uwb Estimated Coordinates', 'green', f'Reference vs Estimated Uwb Coordinates - {titleSuffix}', refAlpha=0.3, estAlpha=1, folderToSave=folderToSave)
        plotSplits(refSplits, modelSplits, 'PixelToReal Estimated Coordinates', 'orange', f'Reference vs Estimated PixelToReal Coordinates - {titleSuffix}', refAlpha=0.3, estAlpha=1, folderToSave=folderToSave)
        plotSplits(refSplits, opticalSplits, 'Optical Estimated Coordinates', 'red', f'Reference vs Estimated Optical Coordinates - {titleSuffix}', refAlpha=0.3, estAlpha=1, folderToSave=folderToSave)
