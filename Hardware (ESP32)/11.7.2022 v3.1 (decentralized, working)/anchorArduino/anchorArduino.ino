...
void setup() {
  ...
  // PRINTLN(mc_addr);
  if (mc_addr == "70:B8:F6:D8:F8:B8") {
    anchorId = 1;
  } else if (mc_addr == "70:B8:F6:D8:F8:28") {
    anchorId = 2;
  } else {
...
void loop() {
  ...
  if (sentFrame) {
    ...
    if (txBuffer[0] == FTYPE_PONG) {
      PRINTLN(F("  PONG sent. Return to IDLE"));
      updateState(STATE_IDLE);
      return;
    }
  ...
  if (receivedFrame) {
    ...
    if (state == STATE_IDLE) {
       ...
       */
        ...
        //delay(d);
       if (!find_link(tagList, sender)) {
         add_link(tagList, sender);
       } else {
         delete_link(tagList, sender);
         return;
       }
        PRINTLN(F("    Reply with PONG"));
        transmitPong();
        updateState(STATE_IDLE);
