...
void loop() {
  ...
  if (state == STATE_ROUNDROBIN) {
    ...
    if (idx_anchor < num_anchors) {
      ...
      if (toSend) updateState(STATE_SEND_DISTANCE_TO_SERVER);
...
//   }
  ...
  if (state == STATE_SEND_DISTANCE_TO_SERVER) {
    ...
    if (client.available()) client.print(all_json);
    // ack = "";
    // delay(30);
    // // while (client.available()) { 
    // //    delay(3);
  ...
  if (state == STATE_IDLE) {
    while (client.available()) request = client.read();
    PRINTLN(request);
  }
