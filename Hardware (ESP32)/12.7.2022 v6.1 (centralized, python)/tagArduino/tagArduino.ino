...
void setAddress()
{
  ...
  if (macAddr == "70:B8:F6:D8:F6:24")
  {
    myID = 1;
  }
  else if (macAddr == "70:B8:F6:D8:F8:B8")
  {
    myID = 2;
  }
...
void loop()
{
  ...
  if (state == STATE_IDLE)
  {
    ...
    while (client.connected())
    {
      ...
      else if (millis() - runtimeDelay > 500)
        break;
    }
    PRINT(F("Ack: "));
    PRINTLN(ack);
  ...
  if (state == STATE_SEND_DISTANCE_TO_SERVER)
  {
    ...
    while (client.connected())
    {
      ...
      else if (millis() - runtimeDelay > 10)
        break;
    }
    if (ack == "55")
    {
  ...
  if (state == STATE_DISCOVERY && ((lastSent && currentTime - lastSent > DISCOVERY_TIMEOUT) || currentTime - lastStateChange > 2 * DISCOVERY_TIMEOUT))
  {

    if (num_anchors > 0)
    {
      PRINT(num_anchors);
      PRINTLN(F(" are detected. Go to RANGING state"));
      idx_anchor = 0;
      updateState(STATE_RANGING);
      return;
    }
    ...
    else
    {
      ...
      if (toSend)
      {
        ...
        for (idx_anchor = 0; idx_anchor < num_anchors; idx_anchor++)
        {
          ...
          {
            if (idx_anchor == 0)
              msgToSend += "{ \"Anchor ID\": " + String(anchors[idx_anchor]) + ", \"distance\": " + String(distances[idx_anchor]) + " }";
            else
              msgToSend += ", { \"Anchor ID\": " + String(anchors[idx_anchor]) + ", \"distance\": " + String(distances[idx_anchor]) + " }";
          }
        }
  ...
  if (state == STATE_POLLACK && ((lastSent && currentTime - lastSent > POLLACK_TIMEOUT) || currentTime - lastStateChange > 2 * POLLACK_TIMEOUT))
  {
  ...
  if (state == STATE_RANGEREPORT && ((lastSent && currentTime - lastSent > RANGEREPORT_TIMEOUT) || currentTime - lastStateChange > 2 * RANGEREPORT_TIMEOUT))
  {
    // toSend = false;
    updateRangingPhase(true);
    return;
  ...
  if (sentAck)
  {
    ...
    if (sentMessageType != BLINK && sentMessageType != POLL && sentMessageType != RANGE)
      return;
