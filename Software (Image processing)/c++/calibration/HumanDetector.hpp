...
public:
    ...
    static void initHumanDetection(const std::string &modelConfiguration, const std::string &modelWeights);
    static std::pair<std::vector<cv::Rect>, std::vector<int>> detectPeople(const cv::Mat &frame);
    static std::pair<std::vector<std::vector<cv::Point2f>>, std::vector<int>> detectAruco(const cv::Mat &frame);
};
